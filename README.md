...
AstrBot 插件：玩家私聊机器人发送 `/注册 密码`，插件自动读取发送者 QQ 号作为游戏账号，校验该 QQ 仍在受管群内后写入 MySQL `user` 表；玩家可私聊 `/找回密码 新密码` 修改自己 QQ 对应账号的密码；玩家退群后自动封号，重新加群后可用 `/解封` 自助解封（自动解封为可选项，默认关闭），并提供定时群成员校验兜底。
...
- 自助解封 `/解封`：玩家重新加入受管群后，可解除本人「因退群被插件自动封禁」的账号；管理员手动封的号会拒绝并提示联系管理员。
- 自动解封（默认关闭）：可选监听 OneBot `group_increase` notice，玩家重新加群时自动解除本插件封的号；需在配置里把 `auto_unban_on_rejoin` 打开才生效。
...
- `auto_unban_on_rejoin`: 是否在重新加群时自动解封（仅解本插件封的号）；**默认关闭**。
...
`/解封` 仅能解除本人「因退群被插件自动封禁」的账号（依据审计表判定）；如果账号是管理员手动封的，或审计表被关闭，插件会拒绝自助解封并提示联系管理员。
